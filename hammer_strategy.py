...
def define_upper_hammer(klines,k=0.4):
    ...
    for index in range(len(klines)):
        ...
        if lower_line > 2 * body and upper_line < k * lower_line:
            # 上涨锤子线出现之前的MA5连续5天下跌,
            if ma[index-5] > ma[index-4] and ma[index-4] > ma[index - 3] and ma[index-3] > ma[index-2] and ma[index-2] > ma[index-1] and ma[index-1] > ma[index]:
                #锤子线的实体不能超过前一天的实体
                if max(klines.iloc[index]['close'],klines.iloc[index]['open']) < max(klines.iloc[index-1]['close'],klines.iloc[index-1]['open']):
                    #锤子线的最低点要低于前5天的最低价,且下影线越长约好
                    if klines.iloc[index]['low'] < klines.iloc[index-1]['low'] and klines.iloc[index]['low'] < klines.iloc[index-2]['low'] and klines.iloc[index]['low'] < klines.iloc[index-3]['low'] and klines.iloc[index]['low'] < klines.iloc[index-4]['low'] and klines.iloc[index]['low'] < klines.iloc[index-5]['low']:
                        hammers.append(Result(klines.iloc[index]['code'],'BUY',klines.iloc[index]['close'],klines.iloc[index]['time_key'],'hammer').get_dict())
    return hammers
            ...
            if satisfy_ma:
                if max(klines.iloc[index]['close'],klines.iloc[index]['open']) > max(klines.iloc[index-1]['close'],klines.iloc[index-1]['open']):
                    ...
                    for body_index in range(days):
                        if body_index == 0:
                            continue
                        satisfy_high = klines.iloc[index]['high'] > max(klines.iloc[index-body_index]['close'],klines.iloc[index-body_index]['open'])
                        if not satisfy_high:
                            break;
                    if satisfy_high:
